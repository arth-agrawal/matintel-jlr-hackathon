...
SCHEMA_ALIASES: dict[str, list[str]] = {
    ...
    "elongation_percent": [
        "elongation", "elongation %", "elong", "elongation_percent",
        "elongation at break",
    ],
    "hardness_hv": [
        "hardness", "hv", "vickers", "vickers hardness", "hrc", "brinell",
    ],
    "fatigue_strength_mpa": [
        "fatigue", "fatigue strength", "endurance limit", "fatigue_strength",
    ],
    "thermal_conductivity_w_mk": [
        "thermal conductivity", "conductivity", "w/mk",
        "thermal_conductivity", "k value",
    ],
    "corrosion_resistance_score": [
        "corrosion", "corrosion resistance", "salt spray", "corrosion score",
    ],
    "recycled_content_percent": [
        "recycled", "recycled content", "recycled %", "pcr content",
        "recycled_content",
    ],
    "co2_kg_per_kg": [
        "co2", "carbon footprint", "embodied carbon", "kgco2",
        "co2/kg", "kg co2e", "co2_kg_per_kg", "gwp",
    ],
    "traceability_score": [
        "traceability", "chain of custody", "origin known",
        "supply chain visibility", "material mapping",
    ],
    "supplier_risk_score": [
        "supplier risk", "supply risk", "sourcing risk",
        "geopolitical risk", "availability risk",
    ],
    "critical_material_risk_score": [
        "critical material", "cobalt", "lithium", "nickel",
        "rare earth", "crm risk", "critical_material",
    ],
    "certification_tags": [
        "certification", "fsc", "iso", "certificate", "certified",
    ],
    "material_name": [
        "name", "material", "alloy", "alloy name", "grade",
    ],
    "formula": [
        "composition", "chemical formula", "chem formula",
    ],
    "material_family": [
        "family", "material family", "material type", "type", "class",
    ],
    "material_subfamily": [
        "subfamily", "sub family", "subclass", "sub type",
    ],
    "cost_index": [
        "cost", "price", "cost index", "price index", "usd/kg",
    ],
    "supplier_name": [
        "supplier", "vendor", "manufacturer",
    ],
    "recyclability_score": [
        "recyclability", "recyclable", "end of life",
    ],
    "bio_based_content_percent": [
        "bio based", "bio content", "biobased", "bio %",
    ],
    "co2_index": [
        "co2 index", "carbon index", "emissions index",
    ],
    "notes": [
        "note", "comment", "comments", "remark", "remarks",
    ],
}
...
UNIT_CONVERSIONS: list[dict] = [
    ...
    {"pattern": r"yield.*pa$", "target": "yield_strength_mpa", "factor": 1e-6},
    {"pattern": r"tensile.*gpa", "target": "ultimate_tensile_strength_mpa", "factor": 1000.0},
    {"pattern": r"modulus.*mpa", "target": "youngs_modulus_gpa", "factor": 0.001},
]
...
def suggest_schema_mapping(uploaded_df: pd.DataFrame) -> pd.DataFrame:
    """Return a DataFrame with columns: uploaded_column, suggested_field, confidence, match_type."""
    rows = []
...
def _detect_unit_conversion(col_name: str, target_field: str) -> float:
    """Return multiplication factor if a unit conversion is needed, else 1.0."""
...
def apply_schema_mapping(
    ...
    key_cols = [
        "yield_strength_mpa", "density_g_cm3", "material_family",
        "source_type", "elongation_percent", "youngs_modulus_gpa",
    ]
    present = [c for c in key_cols if c in out.columns]
    if present:
        out["data_completeness_score"] = (
            100 * (1 - out[present].isna().mean(axis=1))
        ).round(1)
    else:
